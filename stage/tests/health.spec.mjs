...
test.describe('gate report', () => {
  ...
  test('red worktree in Build: the control counts 1; the popover shows exactly one line with the evidence and one fix', async ({ page }, testInfo) => {
    ...
    await page.click(CONTROL)
    const lines = page.locator(`${POP} .skp-report-line`)
    ...
    // The amber "git facts unavailable" hygiene line is UNKNOWN: not in the report.
    await expect(page.locator(POP)).not.toContainText(/Worktrees.*unavailable|Clean up worktrees/u)
  ...
  test('dismiss hides the line for the session and tells the host', async ({ page }) => {
    await openStage(page, 'red-worktree')
    await page.click(CONTROL)
    await page.locator(`${POP} .skp-report-line`).first().getByRole('button', { name: /dismiss/iu }).click()
    await expect(page.locator(`${POP} .skp-report-line`)).toHaveCount(0)
    await expect(page.locator(`${CONTROL} .skp-ctl-count`)).toHaveCount(0)
  ...
  test('the sidebar Skills tab: the same report at the top; unknowns muted under "Not judged"; no amber badges', async ({ page }, testInfo) => {
    ...
    const report = pane.locator('.skp-report-line')
    await expect(report).toHaveCount(1)
    await expect(pane.locator('.skp-not-judged')).toContainText(/Clean up worktrees/u)
